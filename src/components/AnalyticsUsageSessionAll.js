	...
	return (
		<TextContainer>
			<Stack alignment="center">
				<Heading size="small">App Usage Statistics (Forever) </Heading>{" "}
...
export default class AnalyticsUsageSessionFour extends PureComponent {
	...
	async get_usage_data() {
		...
		const sevenDaysAgo = new Date(Date.now() - 10000 * 24 * 60 * 60 * 1000) 
		...
		let response = await fetch(`https://www.primeai.co.uk/DORSIA/project/shopify/admin_rest_apis/get_session_usage.php?shop=dorsiabc&start=1970-1-1end=${now}`, {
	...
	render() {
		...
		return (
			<Card title={<AnalyticsTitle></AnalyticsTitle>} sectioned actions={[{ content: u_week_users(this.state) }, { content: week_users(this.state) }, { content: week_clicks(this.state) },  {content: uploads_week_users(this.state) }]}>
